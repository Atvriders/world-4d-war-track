...
export async function fetchAllSatellites(): Promise<SatelliteEntity[]> {
  try {
    ...
    // If all groups failed and we got nothing, return empty — retry hook will try again
    if (combined.length === 0) {
      console.warn('[Satellites] Offline — will retry. Reason:', 'All CelesTrak groups returned empty');
      return [];
    }

    return combined;
  } catch (err) {
    console.warn('[Satellites] Offline — will retry. Reason:', (err as Error).message);
    return [];
  }
}
